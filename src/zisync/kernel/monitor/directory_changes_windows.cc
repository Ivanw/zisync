...
#include "zisync/kernel/monitor/delayed_directory_change_handler.h"
	...
	****************/
{
		...
		&&	m_pCurrentRecord->NextEntryOffset != 0UL)//is there another record after this one?
	{
		...
		if( (DWORD)((BYTE*)m_pCurrentRecord - m_pBuffer) > m_dwBufferSize )
		{
			...
			//
			// This sometimes happens if the watched directory 
			// becomes deleted... remove the FILE_SHARE_DELETE 
			// flag when using CreateFile() to get the handle 
			// to the directory...
			//
	...
	//	Add the ptr to the first non-null slot in the array.
{
	...
	int i, max = m_DirectoriesToWatch.GetSize();
	for(i = 0; i < max; ++i)
	{
