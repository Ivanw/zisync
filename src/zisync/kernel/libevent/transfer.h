...
class ITaskMonitor {
 public:
  ...
  virtual void OnFileTransfered(int32_t count) = 0;
  virtual void OnFileSkiped(int32_t count) = 0;
...
class IPutHandler {
 public:
  ...
  virtual bool OnHandleFile(
      ...
      const std::string& real_path, const std::string& sha1) = 0;
};
...
class ITreeAgent {
 public:
  ...
  virtual IPutHandler* CreatePutHandler(const std::string &tmp_root) = 0;
  virtual IPutHandler* CreateUploadHandler(
      const std::string &tree_uuid, const std::string &tmp_root) = 0;

  virtual ITaskMonitor* CreateTaskMonitor(
      TaskType type,
      const std::string& local_tree_uuid,
      const std::string& remote_tree_uuid,
      int32_t total_files, int64_t total_bytes) = 0;
};
...
class ITransferServer : public ILibEventVirtualServer {
 public:
  ...
  virtual err_t CancelTaskByTree(int32_t tree_id) = 0;
