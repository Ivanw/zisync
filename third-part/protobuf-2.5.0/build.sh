...
#    set +e
    ./configure $ARCHFLAG --prefix=$PREFIX --enable-shared=no --with-protoc=`which protoc`  > "${LOG}" 2>&1
    #./configure $ARCHFLAG --prefix=$PREFIX --enable-shared=no --with-protoc="$theBinDir/iPhoneSimulator7.1-x86_64.sdk/bin/protoc" > "${LOG}" 2>&1
...
do
    ...
    if [[ ! -h $onelib  && $filename  == "libprotobuf.a"  ]]
    then
...
if [ ! -e "../build/bin"   ] ; then
    mkdir "../build/bin"
    cp $theBinDir/iPhoneSimulator7.1-x86_64.sdk/bin/protoc ../build/bin/protoc
elif [  -e ../build/bin/protoc  ] ; then
    rm ../build/bin/protoc
    cp $theBinDir/iPhoneSimulator7.1-x86_64.sdk/bin/protoc ../build/bin/protoc
else
    cp $theBinDir/iPhoneSimulator7.1-x86_64.sdk/bin/protoc ../build/bin/protoc
...
lipo -create $theLibsToAdd -output $theLibDir/libprotobuf.a
