...
do
  echo "arch is: "$ARCH
    ...
    then
        ...
        ARCHFLAG=" --host=arm-apple-darwin "
    fi
    ...
    then
        ...
        export CPPFLAGS="-arch $ARCH -isysroot $SDKROOT -I$theIncdir"
        export CFLAGS="$CPPFLAGS -miphoneos-version-min=7.0 -std=c99 -pipe -no-cpp-precomp -DSQLITE_HAS_CODEC"
        export CXXFLAGS="$CPPFLAGS -miphoneos-version-min=7.0 -I$SDKROOT/usr/include/c++/4.2.1/$ARCH -pipe -no-cpp-precomp"
        export LDFLAGS="-arch $ARCH -isysroot $SDKROOT -L$theLibDir/"
    fi
...
#    export LDFLAGS="-isysroot $SDKROOT"
	...
	set +e
    ./configure $ARCHFLAG --enable-tempstore=yes --prefix=$PREFIX --enable-shared=no --disable-tcl > "${LOG}" 2>&1
...
theLibsToAdd=""
for  onelib in `find $theBinDir -iname libsqlcipher.a`
do
    if [ ! -h $onelib  ]
    then
...
echo "libs to pack: "$theLibsToAdd
lipo -create $theLibsToAdd -output $theLibDir/libsqlcipher.a
...
do
    ...
    incDirToCp=$theBinDir/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/include/
    cp -r $incDirToCp $theIncdir
    break
done
