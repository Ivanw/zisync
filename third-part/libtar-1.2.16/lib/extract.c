...
tar_set_file_perms(TAR *t, char *realname)
{
	...
	filename = (realname ? realname : th_get_pathname(t, namebuf, MAXPATHLEN));
	mode = th_get_mode(t);
	uid = th_get_uid(t);
	gid = th_get_gid(t);
...
# endif
	...
	if (TH_ISREG(t) && utime(filename, &ut) == -1)
	{
#ifdef DEBUG
		perror("utime()");
#endif
...
tar_extract_file(TAR *t, char *realname)
{
	...
	if (lnp == NULL)
		return -1;
	strlcpy(lnp->ln_save, th_get_pathname(t, namebuf, MAXPATHLEN), sizeof(lnp->ln_save));
