 ...
 *
 * @brief Test cases for content_resolver.cc.
 *
...
using zs::StartupContentFramework;
using zs::ShutdownContentFramework;
using zs::GetContentResolver;
using zs::ContentObserver;
using zs::TableDevice;

TEST(test_ContentResolver) {
  const char* app_path = ".";

  err_t eno = StartupContentFramework(app_path);
  ...
  int64_t rowid = resolver->Insert(TableDevice::URI, &cv, zs::AOC_REPLACE);
  // printf("rowid: %" PRId64 "\n", rowid);
  CHECK(rowid > 0);
  ...
  rowid = resolver->Insert(TableDevice::URI, &cv2, zs::AOC_REPLACE);
  // printf("rowid: %" PRId64 "\n", rowid);
  CHECK(rowid > 0);
  ...
  std::unique_ptr<ICursor2> cursor(
      resolver->Query(TableDevice::URI, projection, ARRAY_SIZE(projection),
                   "uuid = 'uuid1'"));
  ...
  std::unique_ptr<ICursor2> cursor2(
      resolver->sQuery(TableDevice::URI, projection,
  ...
  int num_affected_row = resolver->Update(
      TableDevice::URI, &cv3, "uuid = 'uuid1'");
  ...
  std::unique_ptr<ICursor2> cursor3(
      resolver->Query(TableDevice::URI, projection3, ARRAY_SIZE(projection3),
                   "uuid = 'uuid1'"));
  ...
  select4.Add("uuid2");
  num_affected_row = resolver->sUpdate(TableDevice::URI, &cv4, &select4);
  CHECK_EQUAL(1, num_affected_row);
  ...
  std::unique_ptr<ICursor2> cursor4(
      resolver->Query(TableDevice::URI, projection4, ARRAY_SIZE(projection4),
                   "uuid = 'uuid2'"));
  ...
  // Test Delete()
  num_affected_row = resolver->Delete(TableDevice::URI, "uuid = 'uuid1'");
  CHECK_EQUAL(1, num_affected_row);

  // Test sDelete()
  Selection selection6("uuid = 'uuid2'");
  num_affected_row = resolver->sDelete(TableDevice::URI, &selection6);
  CHECK_EQUAL(1, num_affected_row);

  // Test CleanUp()
  eno = ShutdownContentFramework();
  CHECK_EQUAL(zs::ZISYNC_SUCCESS, eno);
}
 ...
 public:
  ...
  virtual void* OnQueryChange() {
    ...
    return resolver->Query(
        TableDevice::URI, projection4, ARRAY_SIZE(projection4), "uuid = 'uuid1'");
  }

  virtual void  OnHandleChange(void* changes) {
    ICursor2* cursor = static_cast<ICursor2*>(changes);
    CHECK(cursor->MoveToNext());
    CHECK_EQUAL(cursor->GetString(0), std::string("uuid1"));
    CHECK_EQUAL(cursor->GetString(1), std::string("name1"));
...
TEST(test_ContentObserver) {
  ...
  resolver->RegisterContentObserver(TableDevice::URI, true, &observer);
  // Test Insert()
  ContentValues cv(8);
  ...
  resolver->UnregisterContentObserver(TableDevice::URI, &observer);
